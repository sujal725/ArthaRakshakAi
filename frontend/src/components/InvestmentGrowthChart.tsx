import type { InvestmentSeriesPoint } from "@/lib/futureSelf";
...
export function InvestmentGrowthChart({ series }: { series: InvestmentSeriesPoint[] }) {
  ...
  const path = series.map((p, i) => `${i === 0 ? "M" : "L"} ${xAt(i)} ${yAt(p.value)}`).join(" ");
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="h-64 w-full" role="img" aria-label="Investment value, month by month">
      {yTicks.map((t, i) => (
        <g key={i}>
